    ...
    try{$MadelineProto->messages->getPeerDialogs(['peers' => [$user]]);
        $x++;
                ...
                try{$MadelineProto->account->updateUsername(['username'=>$user]);
                    ...
                    bot('sendvideo', ['video' =>'https://telegra.ph/file/c0f0799b66d88e42feeac.mp4', 'chat_id' => file_get_contents("ID"), 'caption' => "⌯ Catch ❲ 9 ❳ New Take 🦅'",]);
                    $data = str_replace("\n".$user,"", file_get_contents("users9"));
                    file_put_contents("users9", $data);
                }catch(Exception $e){
                    echo $e->getMessage();
                    if($e->getMessage() == "USERNAME_INVALID"){
                        bot('sendvideo', ['video' =>'https://telegra.ph/file/c0f0799b66d88e42feeac.mp4', 'chat_id' => file_get_contents("ID"), 'caption' => "Catch ❲ 9 ❳ \n is Band ↣ @$user",]);
                        $data = str_replace($user,"", file_get_contents("users9"));
                        file_put_contents("users9", $data);
